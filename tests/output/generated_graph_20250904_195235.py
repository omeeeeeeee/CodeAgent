...
Credit Report Processing Workflow - H-Test-002
"""
...
async def input_action(state: State, config: RunnableConfig, text: str, description: str, node_number: int) -> State:
    ...
    try:
        ...
        if text.startswith("state."):
            field_name = text[6:]  # Remove "state." prefix
...
async def navigation_extract_borrower_name(state: State, config: RunnableConfig) -> State:
    return await extract_borrower_name(state, config)

async def navigation_click_pipeline(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 85, 60, "Coordinates for Pipeline", 2)

async def navigation_wait_1s(state: State, config: RunnableConfig) -> State:
    return await wait_action(state, config, 1, "Sleep", 3)

async def navigation_click_borrower_input(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 333, 234, "Coordinates for Borrower Name Input Box", 4)

async def navigation_input_name(state: State, config: RunnableConfig) -> State:
    return await input_action(state, config, "state.borrower_name", "NAME", 5)

async def navigation_enter(state: State, config: RunnableConfig) -> State:
    return await enter_action(state, config, "ENTER", 6)

async def navigation_wait_3s(state: State, config: RunnableConfig) -> State:
    return await wait_action(state, config, 3, "Sleep", 7)

async def navigation_double_click_borrower(state: State, config: RunnableConfig) -> State:
    return await double_click_action(state, config, 184, 254, "Coordinates for borrower name from the list", 8)

async def navigation_wait_3s_2(state: State, config: RunnableConfig) -> State:
    return await wait_action(state, config, 3, "Sleep", 9)

async def navigation_click_services(state: State, config: RunnableConfig) -> State:
...
async def main_click_okay(state: State, config: RunnableConfig) -> State:
    # Disabled node - no-op but still update state
    logging.info("Node 7: click_okay disabled - no-op")
...
async def main_click_yes(state: State, config: RunnableConfig) -> State:
    # Disabled node - no-op but still update state
    logging.info("Node 10: click_yes disabled - no-op")
...
def build_navigation_subgraph():
    ...
    # Add nodes
    g.add_node("extract_borrower_name", navigation_extract_borrower_name)
    g.add_node("click_pipeline", navigation_click_pipeline)
    g.add_node("wait_1s", navigation_wait_1s)
    g.add_node("click_borrower_input", navigation_click_borrower_input)
    g.add_node("input_name", navigation_input_name)
    g.add_node("enter", navigation_enter)
    g.add_node("wait_3s", navigation_wait_3s)
    g.add_node("double_click_borrower", navigation_double_click_borrower)
    g.add_node("wait_3s_2", navigation_wait_3s_2)
    g.add_node("click_services", navigation_click_services)
...
def build_return_subgraph():
    """Build return subgraph for cleanup and navigation back to home."""
    g = StateGraph(State)
...
async def finalize_state(state: State, config: RunnableConfig) -> State:
    ...
    logging.info(f"Workflow finalized with status: {state.status}")
    return state
...
def build_main_graph():
    """Build the main graph using compiled subgraphs."""
