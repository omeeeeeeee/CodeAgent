...
class State(BaseModel):
    """State for Credit Report Processing Workflow"""
...
async def input_action(state: State, config: RunnableConfig, text: str, description: str, node_number: int) -> State:
    ...
    try:
        ...
        if text.startswith("state."):
            ...
            text = getattr(state, field_name, text)
        
        await AGENT.input_text(str(text))
...
async def extract_borrower_name(state: State, config: RunnableConfig) -> State:
    """Extract borrower name from user_input"""
    try:
...
async def click_pipeline(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 85, 60, "Coordinates for Pipeline", 2)

async def wait_1s(state: State, config: RunnableConfig) -> State:
    return await wait_action(state, config, 1, "Sleep", 3)

async def click_borrower_input(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 333, 234, "Coordinates for Borrower Name Input Box", 4)

async def input_name(state: State, config: RunnableConfig) -> State:
    return await input_action(state, config, "state.borrower_name", "NAME", 5)

async def enter(state: State, config: RunnableConfig) -> State:
    return await enter_action(state, config, "ENTER", 6)

async def wait_3s(state: State, config: RunnableConfig) -> State:
    return await wait_action(state, config, 3, "Sleep", 7)

async def double_click_borrower(state: State, config: RunnableConfig) -> State:
    return await double_click_action(state, config, 184, 254, "Coordinates for borrower name from the list", 8)

async def wait_3s_2(state: State, config: RunnableConfig) -> State:
    return await wait_action(state, config, 3, "Sleep", 9)

async def click_services(state: State, config: RunnableConfig) -> State:
...
async def click_credit_report(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 391, 60, "Coordinates for Credit Report", 1)

async def wait_5s_2(state: State, config: RunnableConfig) -> State:
    return await wait_action(state, config, 5, "Sleep", 2)

async def click_credit_legacy(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 507, 266, "Coordinates for Advantage Credit Inc Legacy Credit", 3)

async def click_submit(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 846, 545, "Coordinates for Submit Button", 4)

async def wait_5s_3(state: State, config: RunnableConfig) -> State:
    return await wait_action(state, config, 5, "Sleep", 5)

async def click_finish(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 859, 669, "Coordinates Finish Button", 6)

async def click_okay(state: State, config: RunnableConfig) -> State:
    # Disabled node - no-op
    logging.info("Node 7: click_okay (disabled) - Click okay")
    state.current_node = 7
    state.status = "Success"
    return state

async def wait_30s(state: State, config: RunnableConfig) -> State:
    return await wait_action(state, config, 30, "Wait 30 seconds", 8)

async def screenshot(state: State, config: RunnableConfig) -> State:
    return await screenshot_action(state, config, "Take a screenshot and store URL in state", 9)

async def click_yes(state: State, config: RunnableConfig) -> State:
    # Disabled node - no-op
    logging.info("Node 10: click_yes (disabled) - Click yes")
    state.current_node = 10
    state.status = "Success"
    return state

async def click_loan(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 134, 65, "Click loan", 11)

async def click_form_tab(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 28, 438, "Click form tab", 12)

async def click_1003_form(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 77, 540, "Click 1003 form", 13)

async def click_down(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 1350, 541, "Click down", 14)

async def click_import_liability(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 878, 313, "Click import liability", 15)

async def wait_5s_4(state: State, config: RunnableConfig) -> State:
    return await wait_action(state, config, 5, "Wait 5 seconds", 16)

async def click_import(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 825, 598, "Click import", 17)

async def click_ok(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 765, 447, "Click ok", 18)

async def wait_5s_5(state: State, config: RunnableConfig) -> State:
    return await wait_action(state, config, 5, "Wait 5 seconds", 19)

async def click_close(state: State, config: RunnableConfig) -> State:
...
async def click_no(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 743, 443, "Click no", 1)

async def click_pipeline_return(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 81, 60, "Click pipeline", 2)

async def click_dropdown(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 327, 99, "Click dropdown", 3)

async def choose_all(state: State, config: RunnableConfig) -> State:
    return await click_action(state, config, 216, 117, "Choose all", 4)

async def wait_5s_6(state: State, config: RunnableConfig) -> State:
    return await wait_action(state, config, 5, "Wait 5 seconds", 5)

async def click_home(state: State, config: RunnableConfig) -> State:
...
def build_navigation_subgraph():
    """Build navigation subgraph for initial navigation and borrower selection"""
    g = StateGraph(State)
    
    g.add_node("extract_borrower_name", extract_borrower_name)
    g.add_node("click_pipeline", click_pipeline)
    g.add_node("wait_1s", wait_1s)
    g.add_node("click_borrower_input", click_borrower_input)
    g.add_node("input_name", input_name)
    g.add_node("enter", enter)
    g.add_node("wait_3s", wait_3s)
    g.add_node("double_click_borrower", double_click_borrower)
    g.add_node("wait_3s_2", wait_3s_2)
    g.add_node("click_services", click_services)
...
def build_main_workflow():
    """Build main workflow for credit report processing and form completion"""
    g = StateGraph(State)
    
    g.add_node("click_credit_report", click_credit_report)
    g.add_node("wait_5s_2", wait_5s_2)
    g.add_node("click_credit_legacy", click_credit_legacy)
    g.add_node("click_submit", click_submit)
    g.add_node("wait_5s_3", wait_5s_3)
    g.add_node("click_finish", click_finish)
    g.add_node("click_okay", click_okay)
    g.add_node("wait_30s", wait_30s)
    g.add_node("screenshot", screenshot)
    g.add_node("click_yes", click_yes)
    g.add_node("click_loan", click_loan)
    g.add_node("click_form_tab", click_form_tab)
    g.add_node("click_1003_form", click_1003_form)
    g.add_node("click_down", click_down)
    g.add_node("click_import_liability", click_import_liability)
    g.add_node("wait_5s_4", wait_5s_4)
    g.add_node("click_import", click_import)
    g.add_node("click_ok", click_ok)
    g.add_node("wait_5s_5", wait_5s_5)
    g.add_node("click_close", click_close)
...
def build_return_subgraph():
    """Build return subgraph for cleanup and returning to home"""
    g = StateGraph(State)
    
    g.add_node("click_no", click_no)
    g.add_node("click_pipeline_return", click_pipeline_return)
    g.add_node("click_dropdown", click_dropdown)
    g.add_node("choose_all", choose_all)
    g.add_node("wait_5s_6", wait_5s_6)
    g.add_node("click_home", click_home)
    
    # Linear flow
    g.add_edge("__start__", "click_no")
    g.add_edge("click_no", "click_pipeline_return")
    g.add_edge("click_pipeline_return", "click_dropdown")
...
def build_main_graph():
    """Build the main credit report processing graph"""
