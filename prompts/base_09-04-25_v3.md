...
## 🚨 Subgraph Creation Requirements (non-negotiable)

When the JSON contains a `subgraphs` section, you **must** create and compile one `StateGraph` **per subgraph entry**; you **must not** flatten subgraph nodes directly into the main graph.

* For each JSON subgraph (e.g., `"navigation_subgraph"`, `"main_workflow"`, `"return_subgraph"`):

  1. Implement a builder:

     ```python
     def build_navigation_subgraph():
         g = StateGraph(State)
         # add nodes in JSON order
         # g.add_node("...", fn)
         # linear edges __start__ → ... → __end__ OR set_entry_point/set_finish_point
         return g.compile(name="navigation_subgraph")
     ```
  2. Every node function inside a subgraph must set `state.current_node` to its JSON `order`.
  3. Include any **disabled** nodes as no-op functions that still set `current_node` and succeed.
  4. Compile each with **`name=<subgraph_name>` exactly matching the JSON**.

**If the JSON does not include a `subgraphs` section**, still create **three** phase subgraphs by convention so downstream tooling can hook in:

* `navigation_subgraph`: early steps (navigation, input, initial waits)
* `main_workflow`: core task steps (service actions, screenshots, form actions)
* `return_subgraph`: cleanup/return steps

Use obvious ordering heuristics from node names and/or numeric ranges (e.g., return steps often have 100-series orders). Document your choice in code comments.

---

## Main Graph + Finalizer (must use compiled subgraphs)

* Build a main `StateGraph(State)` and **add each compiled subgraph** as nodes named exactly as the JSON keys (or the conventional names above).
  **Do not add raw step functions directly to the main graph.**
...
* The final compiled graph **must always** be assigned to a top-level variable named `graph`.
  ...
  ```
* Do not use other variable names. `langgraph dev` expects `agent: .graph.py`.
...
* [ ] **Each JSON subgraph is compiled** via `StateGraph(...).compile(name=<subgraph_name>)`.
* [ ] **Main graph only contains subgraph nodes** (+ `finalize_state`), not raw step functions.
* [ ] Main graph compiled with `GRAPH_NAME`; module exports `graph` at top level.
